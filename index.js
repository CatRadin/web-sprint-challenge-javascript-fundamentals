...
// The nestedFunction() has access the variable internal that is in myFunction() because of closure! It inherits the variable. The child function nestedFunction() has access to everything it's parent function myFunction() has.
...
const zooAnimals = [
 ...
 function animalNames(animalArray){
  const displayNames = [];
  animalArray.forEach(ele => displayNames.push(`name: ${ele.animal_name}, scientific: ${ele.scientific_name}`));
  return displayNames;
}
  ...
  */
  ...
  function lowerCaseNames(animalArray){
    return animalArray.map(ele => ele.animal_name.toLowerCase());
  }
  ...
  */
  ...
  function lowPopulationAnimals(animalArray){
   return animalArray.filter(ele => ele.population < 5);  
  }
  ...
  */
  ...
  function USApop(animalArray){
    return animalArray.reduce((sum, ele) => sum += ele.population, 0);
  }
...
//I've be trying hard to do arrow functions only! :) 
...
class CubeMaker extends CuboidMaker {
  ...
  surfaceArea() {
    return 6 * Math.pow(this.length, 2) // ??? I think
  }
}
